...
export const POST: APIRoute = async ({ request }) => {
    const SECRET_KEY = import.meta.env.FIUU_SECRET_KEY;
    ...
    const { tranID, orderid, status, domain, amount, currency, appcode, paydate, skey } = data;

    // --- Log callback ---
    console.log('');
    console.log('╔══════════════════════════════════════════════════════╗');
    console.log('║         FIUU PAYMENT CALLBACK RECEIVED               ║');
    console.log('╠══════════════════════════════════════════════════════╣');
    console.log('║ Order ID:      ', orderid);
    console.log('║ Status:        ', status, status === '00' ? '(SUCCESS)' : '(FAILED/PENDING)');
    ...
    // --- Verify skey to ensure callback is genuinely from FIUU ---
    if (SECRET_KEY && skey) {
        const key0 = md5Pure(tranID + orderid + status + domain + amount + currency);
        const key1 = md5Pure(paydate + domain + key0 + appcode + SECRET_KEY);
    ...
    // --- Only process successful payments (status "00") ---
    if (status === '00' && SHEET_WEBHOOK) {
        try {
...
// --- Pure JS MD5 (same as checkout.ts) ---
